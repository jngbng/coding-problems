...
class SegmentTree {
  ...
  constructor(arr, n) {
    ...
    const x = parseInt(Math.ceil(Math.log(n) / Math.log(2)));
    
    // Maximum size of segment tree
    const max_size = 2 * parseInt(Math.pow(2, x) - 1);
  ...
  /* A recursive function to update the nodes which have the given index in their range. 
    The following are parameters st, si, ss and se are same as getSumUtil() i --> index
    of the element to be updated.This index is in input array. diff --> Value
...
const n = arr.length;
const tree = new SegmentTree(arr, n);

console.log(tree.getSum(n, 1, 3));
